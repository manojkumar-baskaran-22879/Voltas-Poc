...
import CreateSalesReturnModal from './CreateSalesReturnModal'; // Ensure correct path
...
const SalesReturnDetails = () => {
  ...
  const handleUpdateSuccess = () => {
    fetchDetails(); // Refresh the details on the same page
  };
    ...
    <div className="min-h-screen bg-white">
      <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
        ...
        <button 
          ...
          className="bg-[#0066b2] hover:bg-blue-700 text-white px-6 py-1.5 rounded-md text-sm font-medium transition-colors"
        >
          Edit
        </button>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-y-4 gap-x-20">
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-start">
                <span className="w-40 text-slate-500 text-sm">Email Opt Out</span>
                <span className="text-slate-400 mr-4">:</span>
                <input type="checkbox" checked={data.Email_Opt_Out || false} readOnly className="mt-1 h-4 w-4 rounded border-gray-300 text-blue-600" />
              </div>
            </div>
          </div>
        </section>
      ...
      <CreateSalesReturnModal 
        ...
        editData={data} // Passing the current data for editing
      />
    </div>
  );
};
