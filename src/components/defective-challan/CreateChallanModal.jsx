  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/20 backdrop-blur-[6px]">
      <div className="bg-white w-full max-w-[550px] rounded-[24px] shadow-2xl border border-white overflow-hidden">
        ...
        {/* Footer - Mobile Responsive Alignment */}
<div className="px-6 sm:px-10 py-6 sm:py-8 flex flex-col sm:flex-row gap-4 items-center justify-between bg-white border-t border-slate-50">
  
  {/* Left side: Previous button (Full width on mobile, auto on desktop) */}
  <div className="w-full sm:w-auto order-2 sm:order-1">
    {step === 2 && (
      <button 
        onClick={() => setStep(1)} 
        className="flex items-center justify-center gap-1.5 text-[#00579c] text-sm font-bold bg-[#f1f6fa] w-full sm:w-auto px-6 py-2.5 rounded-full hover:bg-slate-100 transition-colors"
      >
        <ChevronLeft size={16} strokeWidth={3} /> Previous
      </button>
    )}
  ...
  {/* Right side: Cancel and Action buttons (Side-by-side on all screens) */}
  <div className="flex items-center justify-center gap-6 w-full sm:w-auto order-1 sm:order-2">
    <button 
      onClick={onClose} 
      className="text-[#00579c] text-sm font-bold hover:underline py-2"
    >
      Cancel
    </button>
    ...
    <button 
      onClick={step === 1 ? () => setStep(2) : handleSubmit}
      disabled={loading}
      className="flex-1 sm:flex-none bg-[#00579c] hover:bg-[#004a85] text-white px-10 py-2.5 rounded-full text-sm font-bold shadow-lg disabled:opacity-50 min-w-[120px]"
    >
      {loading ? (editData ? 'Updating...' : 'Saving...') : (step === 1 ? 'Next' : (editData ? 'Update' : 'Create'))}
    </button>
  </div>
