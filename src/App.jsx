...
function AppLayout({ isSidebarOpen, setSidebarOpen, email, isAdmin = true }) {
  ...
  const navItems = [
    ...
    { icon: ClipboardList, label: "Service Requests", to: "/service-requests" },
    { icon: Calculator, label: "Estimations", to: "/estimations" },
    { icon: Package, label: "Agency Wise Stock", to: "/agency-stock" },
    { icon: RotateCcw, label: "Sales Return Order", to: "/sales-return" },
    { icon: FileWarning, label: "Defective Challan", to: "/defective-challan" },
    { icon: Wrench, label: "Work Order", to: "/work-orders" },
    { icon: Truck, label: "Order Receiving", to: "/order-receiving" },
  ];
...
/* =======================
   NAV ITEM
======================= */
function NavLink({ icon: Icon, label, to, setSidebarOpen }) {
  const handleClick = () => {
    // Check if current view is mobile (breakpoint 1024px)
